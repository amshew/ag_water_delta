...
library(sp)
library(spdep)
coordinates(IWM_costshare_merged) <- ~longitude + latitude
plot(IWM_costshare_merged)
colnames(IWM_costshare_merged)


proj4string(IWM_costshare_merged) <- CRS("+proj=aea +lat_1=34 +lat_2=40.5 +lat_0=0 +lon_0=-120 +x_0=0 +y_0=-4000000 +datum=NAD83 +units=km +ellps=GRS80")



w <- 1/as.matrix(dist(coordinates(IWM_costshare_merged)))

diag(w) <- 0
moran.test(IWM_costshare_merged$lcost_share,mat2listw(w))

COUNTYFP10<-"10"
year<-"2010"
moran_I <- function(COUNTYFP10, year) {
  Data <- IWM_costshare_sum_practice<-IWM_costshare_merged%>%group_by(County,Year,longitude,latitude)%>%summarise(Practice_cost = sum(cost_share))%>%
    merge(.,County, by=c("County", "Year"="Year"),sort = TRUE)
  #print(Data)
 
  Data<-  Data[ Data$Year==year&Data$COUNTYFP10==COUNTYFP10, ]
