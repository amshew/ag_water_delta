...
get_save_prism_y <- function(temp_start_year, var_type) {
  ...
  temp_stars <- stack(file_path) %>% 
    #--- crop to AR ---#
    crop(., extent(AR.Delta_d)) %>% 
    mask (., AR_huc12)%>% 
    raster::extract(.,AR_huc12, progress = F,
  ...
  saveRDS(
    tmax_long, 
    paste0('C:/Users/obemb/OneDrive/Documents/prismtmp/Weather Data/PRISM_', var_type, "_y", temp_start_year, ".rds")
  )
